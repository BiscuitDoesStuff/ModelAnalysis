...
# L1 must have an OR listing; L0 must be AA $0 without OR listing.
...
check(all(not m.get("or_id") and not m.get("providers") for m in l0),
      f"L0 all AA-only, no callable ({len(l0)})")
...
for t in ("max", "high", "medium"):
    for m in r["ocf_stack"][t]["rows"]:
        check(bool(m.get("or_id")) or bool(set(m.get("providers", [])) & {"openai", "anthropic", "openrouter"}),
              f"stack[{t}] callable only")
        if not (bool(m.get("or_id")) or bool(set(m.get("providers", [])) & {"openai", "anthropic", "openrouter"})):
            break
...
# MD shows [F?] marker; HTML shows it too.
if m_files:
